...
## Course: CIS 344 Database Concepts
## file name: restaurantServer.py
    ...
    def do_GET(self):
        ...
        try:
            if self.path == '/':
                ...
                #Menu Block Starts - Add Reservations
                self.wfile.write(b"<html><head><title>Add Reservations</title></head>")
                self.wfile.write(b"<body>")
                self.wfile.write(b"<center><h1>Add Reservations</h1>")
                self.wfile.write(b"<hr>")
                self.wfile.write(b"<div> <a href='/'>Home</a>| \
                                 <a href='/addReservation'>Add Reservation</a>|\
                                  <a href='/deleteReservation'>Delete Reservation</a>|\
                                  <a href='/viewReservations'>View Reservations</a></div>")
                self.wfile.write(b"<hr><h2></h2>")
                #Menu Block Ends - Add Reservations
                ...
                self.wfile.write(b"<label>Customer Name: </label>(Name of person booking the reservation?</label> ")
                self.wfile.write(b"<input type='text' name='customer_name' required><br>")
                self.wfile.write(b" ")
                self.wfile.write(b"<label>Contact Info: </label> <label>(An email or mobile number is fine.)</label> ")
                self.wfile.write(b"<input type='text' name='contact_info' required><br>")
                
                self.wfile.write(b"<label>Reservation Time: </label> <label>(Time format: YYYY-MM-DD HH:MM:SS)</label> ")
                self.wfile.write(b"<input type='text' name='reservation_time' required><br>")
                
                self.wfile.write(b"<label>Number of Guests: </label>(How many people are coming?)</label> ")
                self.wfile.write(b"<input type='text' name='number_of_guests' required><br>")
                
                self.wfile.write(b"<label>Special Requests: </label> <label>(Any special request? An allergy maybe?)</label> ")
                ...
                #Menu Block Starts - Delete Reservations
                self.wfile.write(b"<html><head><title>Delete Reservations</title></head>")
                self.wfile.write(b"<body>")
                self.wfile.write(b"<center><h1>Delete Reservations</h1>")
                self.wfile.write(b"<hr>")
                self.wfile.write(b"<div> <a href='/'>Home</a>| \
                                 <a href='/addReservation'>Add Reservation</a>|\
                                  <a href='/deleteReservation'>Delete Reservation</a>|\
                                  <a href='/viewReservations'>View Reservations</a></div>")
                self.wfile.write(b"<hr><h2></h2>")
                #Menu Block Ends - Delete Reservations
                
                self.wfile.write(b"<html><head><title>Delete Reservation</title></head>")
                self.wfile.write(b"<body>")
                self.wfile.write(b"<center><h2>Delete Reservation</h2>")
                self.wfile.write(b"<form method='post' action='/deleteReservation'>")
                self.wfile.write(b"<label>Reservation ID to delete: </label><label>(Please input the ID you wish to delete)</label> ")
                ...
                #Menu Block Starts - View Reservations 
                ...
                #Menu Block Ends - View Reservations 
